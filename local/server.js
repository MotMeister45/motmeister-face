...
// Sync state to public face via GitHub Gist (instant updates!)
const GIST_ID = '699e9c29d73186c38ead75eaf98561cd';
...
function syncPublicState(state) {
  ...
  syncTimeout = setTimeout(() => {
    // Get weather
    exec('/usr/bin/curl -s "wttr.in/Manhattan,NYC?format=%c+%t+%C"', (err, stdout) => {
      ...
      // Update GitHub Gist (instant, no caching)
      const escaped = stateJson.replace(/'/g, "'\\''");
      const cmd = `echo '${escaped}' | /opt/homebrew/bin/gh gist edit ${GIST_ID} -f state.json -`;
      exec(cmd, (err, stdout, stderr) => {
        if (!err) {
          console.log(`📡 Gist synced: ${state}`);
        } else {
          console.error('Gist sync failed:', stderr || err.message);
        }
      });
    });
  }, 300); // Faster debounce for quicker updates
}
